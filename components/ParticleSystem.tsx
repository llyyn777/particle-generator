...
interface ParticleSystemProps {
  shapeType: ShapeType;
  generatedPoints: Vector3[] | null;
  color: string;
  handOpenness: number; // 0 to 1
  particleSize: number;
}

const ParticleSystem: React.FC<ParticleSystemProps> = ({ shapeType, generatedPoints, color, handOpenness, particleSize }) => {
    ...
    switch (shapeType) {
      ...
      case ShapeType.GEMINI_GENERATED:
        if (generatedPoints && generatedPoints.length > 0) {
          // Upscale the 500 points to PARTICLE_COUNT by duplicating with jitter
          points = [];
          for(let i=0; i<PARTICLE_COUNT; i++) {
            const basePoint = generatedPoints[i % generatedPoints.length];
            points.push({
              x: basePoint.x + (Math.random() - 0.5) * 0.2,
              y: basePoint.y + (Math.random() - 0.5) * 0.2,
              z: basePoint.z + (Math.random() - 0.5) * 0.2
            });
          }
        } else {
           points = generateSphere(PARTICLE_COUNT); // Fallback
        }
        break;
      default:
        points = generateSphere(PARTICLE_COUNT);
