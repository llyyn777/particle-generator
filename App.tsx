...
import { generateShapeFromGemini } from './services/geminiService';
...
const App: React.FC = () => {
  ...
  const [generatedPoints, setGeneratedPoints] = useState<Vector3[] | null>(null);
  ...
  const handleGeneratePrompt = async (prompt: string) => {
    ...
    try {
      const points = await generateShapeFromGemini(prompt);
      setGeneratedPoints(points);
      setCurrentShape(ShapeType.GEMINI_GENERATED);
    } catch (error) {
      alert("Failed to generate shape. Check API Key configuration.");
    } finally {
